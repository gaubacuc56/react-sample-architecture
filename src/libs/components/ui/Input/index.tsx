...
const Input = memo(
	forwardRef<HTMLInputElement, IInputProps>((props: IInputProps, ref) => {
		const {
			id,
			extra,
			type,
		...
		const _inputClasses = useMemo(() => {
			...
			let inputClasses = `flex h-11 w-full items-center justify-center rounded-md border border-gray-300 bg-white/0 p-3 text-sm outline-none ${_extraClass}`;
		...
		return (
			<div className={`${extra}`}>
				<div className="relative" onClick={onClick}>
					{leftIcon !== undefined ? (
						<span className={`${_iconClasses} left-0 pl-3`}>
							{leftIcon}
						</span>
					) : (
						""
					...
					{rightIcon !== undefined ? (
						<span
							className={`${_iconClasses} right-0 pr-3 cursor-pointer`}
						>
							{rightIcon}
						</span>
					) : (
